...
def markAttendance(name,frame):
    ...
    g=open(f'Attendance/{dateString}/Attendance_{dateString}_IN.log', 'r+')
    g1=open(f'Attendance/{dateString}/Attendance_{dateString}_OUT.log', 'r+')
    ...
    if name not in nameList:
        ...
        engine.runAndWait()
        value = write_read()
        f.writelines(f'{name},{dtString},{str(value)[2:6]}\n')
        ...
        engine.runAndWait()
        print("Your body temperature is",str(value)[2:6],"*F")
        engine.say("Your body temperature is "+str(value)[2:6]+" degree Fahrenheit")
        engine.runAndWait()
